...
def get_correspondencies(dataframe_shapes_automatically_produced: Union[pd.DataFrame, gpd.GeoDataFrame],
    ...
    :param dataframe_shapes_mannually_produced: must_contain columns `ID`, `uuid`, `geometry`
    ...
    for i in tqdm(range(len(centroids_manual))):
        ...
        # Get the row corresponding to the manual geometry
        ...
        df_correspondances.loc[index, 'manual_ID'] = int(row_manual.ID) if not pd.isnull(row_manual.ID) else -1
