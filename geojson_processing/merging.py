...
def concat_geojson_files(list_geojson_files: List[str],
    ...
    for filename in tqdm(list_geojson_files):
        ...
        id_map_sheet = re.search('[0-9]{2}', basenme).group()
        tif_filename_list = glob(os.path.abspath(os.path.join(geotif_files_directory, '*{}*.tif*'.format(id_map_sheet))))
        if len(tif_filename_list) != 1:
            raise ValueError('There are more than 1 item with id {}'.format(id_map_sheet))
        else:
